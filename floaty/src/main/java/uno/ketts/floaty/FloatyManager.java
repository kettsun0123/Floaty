...
class FloatyManager {
    ...
    public void show(Scene scene, int duration, Callback callback) {
        synchronized (lock) {
            if (isCurrentLocked(callback)) { //すでに同一のFloatyが表示されている場合
                current.duration = duration;
                handler.removeCallbacksAndMessages(current);
                scheduleTimeoutLocked(current);
                return;
            } else if (isNextLocked(callback)) { //次に表示される予定のFloatyが同一の場合
            ...
            if (current != null && cancelLocked(current, Floaty.Callback.DISMISS_EVENT_CONSECUTIVE)) {
                ...
                // ifの条件式ないでcancelLockがコールされ、すでに表示されているFloatyはhideViewされる
                current = null;
                showNextLocked();
            }
        }
    ...
    public void pauseTimeout(Callback callback) {
        synchronized (lock) {
            if (isCurrentLocked(callback) && !current.paused) {
                current.paused = true;
                handler.removeCallbacksAndMessages(current);
            }
        }
